...
using LincsProject, DataFrames, CSV, Dates, JSON, StatsBase, JLD2, SparseArrays, Dates, Printf, Profile, Zygote
...
const additional_notes = "demo 10ep for ALiBi implementation, batchsize lowered, softmax over dims=1 for k"
...
# # cut to top 200
# const top_k_genes = 200
# X = X_full_ranked[1:top_k_genes, :]
...
function get_alibi_slopes(n_heads::Int)
    # For n heads, we use slopes: 2^(-8/n), 2^(-16/n), ..., 2^(-8)
    # This ensures geometric sequence of slopes
    
    # Get the closest power of 2
    closest_power_of_2 = 2^floor(Int, log2(n_heads))
    
    # Generate base slopes (for power of 2 number of heads)
    base_slopes = [2.0f0^(-8.0f0 * i / closest_power_of_2) for i in 1:closest_power_of_2]
    
    if closest_power_of_2 < n_heads
        # If not a power of 2, we need extra slopes
        # Use the geometric mean to fill in
        extra = n_heads - closest_power_of_2
        extra_slopes = [2.0f0^(-4.0f0 * i / extra) for i in 1:extra]
        slopes = vcat(base_slopes, extra_slopes)
    else
        slopes = base_slopes
    end
    
    return cu(Float32.(slopes))
end

### Custom Multi-Head Attention with ALiBi
struct CustomMultiHeadAttention
    qkv_proj::Flux.Dense
    out_proj::Flux.Dense
    attn_drop::Flux.Dropout
    n_heads::Int
    head_dim::Int
    alibi_slopes::CuArray{Float32, 1}  # Store slopes as part of the layer
end

function CustomMultiHeadAttention(embed_dim::Int; n_heads::Int, dropout_prob::Float64)
    @assert embed_dim % n_heads == 0 "embed_dim must be divisible by n_heads"
    head_dim = embed_dim ÷ n_heads
    
    qkv_proj = Flux.Dense(embed_dim => 3 * embed_dim, bias=false)
    out_proj = Flux.Dense(embed_dim => embed_dim, bias=false)
    attn_drop = Flux.Dropout(dropout_prob)
    alibi_slopes = get_alibi_slopes(n_heads)
    
    return CustomMultiHeadAttention(qkv_proj, out_proj, attn_drop, n_heads, head_dim, alibi_slopes)
end

Flux.@functor CustomMultiHeadAttention (qkv_proj, out_proj, attn_drop)

function batched_matmul_4d(a::CuArray{T, 4}, b::CuArray{T, 4}) where T
    # a: (d1, d2, n_heads, batch_size)
    # b: (d2, d3, n_heads, batch_size)
    # output: (d1, d3, n_heads, batch_size)
    d1, d2, n_heads, batch_size = size(a)
    _, d3, _, _ = size(b)
    
    a_reshaped = reshape(a, d1, d2, n_heads * batch_size)
    b_reshaped = reshape(b, d2, d3, n_heads * batch_size)
    c = NNlib.batched_mul(a_reshaped, b_reshaped)
    
    return reshape(c, d1, d3, n_heads, batch_size)
end

function (mha::CustomMultiHeadAttention)(x::CuArray{Float32, 3})
    embed_dim, seq_len, batch_size = size(x)
    
    # Project to Q, K, V
    qkv = mha.qkv_proj(reshape(x, embed_dim, :))
    qkv = reshape(qkv, 3 * embed_dim, seq_len, batch_size)
    ...
    # Reshape for multi-head attention: (head_dim, seq_len, n_heads, batch_size)
    q = reshape(q, mha.head_dim, mha.n_heads, seq_len, batch_size)
    k = reshape(k, mha.head_dim, mha.n_heads, seq_len, batch_size)
    v = reshape(v, mha.head_dim, mha.n_heads, seq_len, batch_size)
    
    # Permute to (head_dim, seq_len, n_heads, batch_size)
    q = permutedims(q, (1, 3, 2, 4))
    k = permutedims(k, (1, 3, 2, 4))
    v = permutedims(v, (1, 3, 2, 4))
    
    # Transpose k for attention: (seq_len, head_dim, n_heads, batch_size)
    k_t = permutedims(k, (2, 1, 3, 4))
    
    # Compute attention scores: (seq_len_q, seq_len_k, n_heads, batch_size)
    scores = batched_matmul_4d(k_t, q)
    scores = scores ./ sqrt(Float32(mha.head_dim))
    
    # Add ALiBi bias
    # Create position indices: [0, 1, 2, ..., seq_len-1]
    positions = cu(Float32.(0:seq_len-1))
    
    # Compute relative position bias: distance between query position i and key position j
    # bias[i,j] = -(i - j) for i >= j (attending to past), or large negative for i < j (future)
    # Shape: (seq_len_q, seq_len_k)
    relative_positions = reshape(positions, seq_len, 1) .- reshape(positions, 1, seq_len)
    relative_positions = abs.(relative_positions)  # |i - j|
    
    # Apply slopes: each head has a different slope
    # Reshape slopes for broadcasting: (1, 1, n_heads)
    slopes_reshaped = reshape(mha.alibi_slopes, 1, 1, mha.n_heads)
    
    # ALiBi bias: -m * |i-j|
    # Shape: (seq_len_q, seq_len_k, n_heads)
    alibi_bias = -relative_positions .* slopes_reshaped
    
    # Add to scores (broadcasts over batch dimension)
    # scores: (seq_len_q, seq_len_k, n_heads, batch_size)
    # alibi_bias: (seq_len_q, seq_len_k, n_heads) -> broadcasts to (seq_len_q, seq_len_k, n_heads, batch_size)
    scores = scores .+ reshape(alibi_bias, seq_len, seq_len, mha.n_heads, 1)
    
    # Apply softmax over the key dimension (first dimension)
    attn_weights = softmax(scores, dims=1)
    attn_weights = mha.attn_drop(attn_weights)
    
    # Apply attention to values: (head_dim, seq_len, n_heads, batch_size)
    out = batched_matmul_4d(v, attn_weights)
    
    # Permute back and reshape to (embed_dim, seq_len, batch_size)
    out = permutedims(out, (1, 3, 2, 4))
    out = reshape(out, embed_dim, seq_len, batch_size)
    
    # Output projection
    out = mha.out_proj(reshape(out, embed_dim, :))
    out = reshape(out, embed_dim, seq_len, batch_size)
...
function Transf(
    ...
    #                                 )
    mha = CustomMultiHeadAttention(embed_dim; n_heads=n_heads, dropout_prob=dropout_prob)
...
function (tf::Transf)(input::Float32Matrix3DType) # input shape: embed_dim × seq_len × batch_size
    ...
    # atted = tf.mha(normed, normed, normed; mask=alibi_bias)[1]  # outputs a tuple (a, b)
    # atted = tf.mha(normed, alibi_bias)
...
function Model(;
    ...
    return Model(embedding, transformer, classifier)
end
...
function (model::Model)(input::IntMatrix2DType)
    ...
    # get alibi
    # seq_len = size(embedded, 2)
    # current_alibi_bias = Zygote.ignore() do
    #     model.alibi_bias[1:seq_len, 1:seq_len, :, :]
    # end

    transformed = model.transformer(embedded)
    # # do alibi per layer
    # transformed = embedded
    # for layer in model.transformer
    #     transformed = layer(transformed, current_alibi_bias)
    # end
...
const seq_len = size(X, 1) # (n_genes + 1 for CLS)
...
function get_profile_embedding(m, input)
    transformed = m.transformer(m.embedding(input))
