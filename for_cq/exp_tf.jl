...
using Flux, Random, ProgressBars, CUDA, Statistics, CairoMakie
...
# untrt only
const data_path = "data_expr.jld2"
...
# notes
const gpu_info = "this was on rorqual"
const dataset_note = "trt"
const additional_notes = "demo cq run"
...
data = load(data_path)["data_expr"]

@time X = data #!# use raw expression values!!!
...
function Model(;
    ...
    return Model(projection, transformer, classifier)
end
...
# heatmap
bin_edges = 1:979 
h = fit(Histogram, (trues_ranked, preds_ranked), (bin_edges, bin_edges))
